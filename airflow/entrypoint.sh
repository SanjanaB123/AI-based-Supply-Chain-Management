...
case "$COMMAND" in
  "db-init")
    ...
    # Use AIRFLOW_ADMIN_PASSWORD from Secret Manager; fall back to 'admin' only for local dev
  ...
  "scheduler")
    ...
    # Port 8974 matches the Cloud Run container port and health check probe
    wait_for_db
    python -m http.server 8974 --bind 0.0.0.0 &
