  ...
  return (
    <div className="space-y-3 h-92 overflow-x-auto">
      ...
      <div className="flex items-center gap-4 text-[11px] text-slate-400 dark:text-slate-500">
        {totalPos > 0 && (
          <span>
            <span className="font-semibold text-amber-600 dark:text-amber-400">{totalPos}</span> with surplus
          </span>
        )}
        {totalNeg > 0 && (
          <span>
            <span className="font-semibold text-slate-500 dark:text-slate-400">{totalNeg}</span> anomaly
          </span>
        )}
