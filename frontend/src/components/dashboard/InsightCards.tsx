...
const ACCENT: Record<AccentColor, { bg: string; border: string; label: string; dot: string }> = {
  emerald: { bg: 'bg-emerald-100', border: 'border-emerald-200', label: 'text-emerald-600', dot: 'bg-emerald-500' },
  red:     { bg: 'bg-red-100',     border: 'border-red-200',     label: 'text-red-600',     dot: 'bg-red-500'     },
  amber:   { bg: 'bg-amber-100',   border: 'border-amber-200',   label: 'text-amber-600',   dot: 'bg-amber-500'   },
  blue:    { bg: 'bg-blue-100',    border: 'border-blue-200',    label: 'text-blue-600',    dot: 'bg-blue-500'    },
};
...
export default function InsightCards({ sellThrough, daysOfSupply, shrinkage }: Props) {
  ...
  if (insights.length === 0) {
    return (
      <div className="flex items-center justify-center py-8 text-sm text-slate-400 h-86">
  ...
  return (
    <div className="space-y-2.5 h-86">
        ...
        return (
          ...
          >
            <div className="mb-1.5 flex items-center gap-2">
              ...
              <p className={`text-[9px] font-semibold uppercase tracking-widest ${s.label}`}>
                {insight.title}
              </p>
            </div>
            <p className="truncate text-[13px] font-semibold text-slate-800">{insight.value}</p>
            <p className="mt-0.5 truncate text-[11px] text-slate-500">{insight.sub}</p>
          </div>
        );
      })}
    </div>
