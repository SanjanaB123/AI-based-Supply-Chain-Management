  ...
  return (
    ...
    >
      ...
      <div className="mb-2 flex items-center justify-between">
        ...
        {isAtRisk ? (
          <span className="ml-2 shrink-0 rounded-full bg-red-100 px-2.5 py-0.5 text-[10px] font-semibold text-red-600">
            AT RISK
          </span>
        ) : isBorder ? (
          <span className="ml-2 shrink-0 rounded-full bg-amber-100 px-2.5 py-0.5 text-[10px] font-semibold text-amber-600">
            +{gap}d buffer
          </span>
        ) : (
          <span className="ml-2 shrink-0 text-[10px] text-slate-400 tabular-nums">
            +{gap}d
          </span>
        )}
      </div>
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-center gap-2">
          <span className="w-11 shrink-0 text-[10px] text-slate-400 text-right">Supply</span>
        ...
        {/* Lead time bar */}
        <div className="flex items-center gap-2">
          <span className="w-11 shrink-0 text-[10px] text-slate-400 text-right">Lead T.</span>
          <div className="flex-1 h-1.5 rounded-full bg-slate-100 overflow-hidden">
            <div
...
export default function LeadTimeRiskModule({ data }: Props) {
  ...
  // Sort by risk: smallest gap (supply − lead time) first
  ...
  return (
    <div className="space-y-4">
      ...
      {atRisk.length > 0 ? (
        <div className="flex items-start gap-3 rounded-lg border border-red-200 bg-red-100 px-4 py-3 text-sm text-red-700">
        ...
        <div className="flex items-center gap-3 rounded-lg border border-emerald-200 bg-emerald-100 px-4 py-3 text-sm text-emerald-700">
      ...
      <div className="flex items-center gap-4 text-[10px] text-slate-400">
        ...
        <span className="ml-auto text-[10px] text-slate-400">sorted by risk</span>
      </div>
