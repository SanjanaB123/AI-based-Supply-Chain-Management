...
const HEALTH_STYLES: Record<Health, { rowBg: string; dayText: string; barColor: string }> = {
  critical: { rowBg: 'bg-red-100/60 dark:bg-red-900/20',   dayText: 'text-red-700 dark:text-red-400',    barColor: '#ef4444' },
  low:      { rowBg: 'bg-amber-100/60 dark:bg-amber-900/20', dayText: 'text-amber-700 dark:text-amber-400',  barColor: '#f59e0b' },
  healthy:  { rowBg: '',                                    dayText: 'text-emerald-700 dark:text-emerald-400', barColor: '#10b981' },
};
      ...
      <div className="flex items-center gap-5 text-sm">
        ...
        {lowCount > 0 && (
          <span className="flex items-center gap-1.5">
            ...
            <span className="font-semibold text-amber-700 dark:text-amber-400">{lowCount}</span>
        ...
        {criticalCount === 0 && lowCount === 0 && (
          <span className="flex items-center gap-1.5">
            ...
            <span className="text-slate-500 dark:text-slate-400">All products adequately stocked</span>
          </span>
        )}
              ...
              {products.map(p => {
                ...
                return (
                  <tr key={p.product_id} className={styles.rowBg}>
                    ...
                    <td className="hidden px-3 py-2.5 text-slate-500 dark:text-slate-400 sm:table-cell">
                      {p.category}
                    </td>
                    <td className="px-3 py-2.5">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <span className={`w-9 text-right text-sm font-semibold tabular-nums ${styles.dayText}`}>
